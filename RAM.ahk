...
/************************************************************************
 * @description Class for automation of RAM
 * @file RAM.ahk
 * @author noks
 * @date 2024/12/07
 * @version 1
...
Class RAM {
    __New(port, WebServerPass){
        ...
        if port < 0 || port > 65535
            Throw TypeError("Expected a port in the range of 0-65535 but instead recived " port)
        this.port := port
    ...
    LaunchAccount(Account,Jobid) {
        ;Launches a acc 
        url := 'http://localhost:' this.port '/LaunchAccount?Account=' Account '&PlaceId=' 1537690962 '&JobId=' Jobid '&Password=' this.WebServerPass
        Whr := ComObject("WinHTTP.WinHTTPRequest.5.1")
        Whr.Open("Get",url)
        ;establish a connection
        try Whr.Send()
        catch ;Failed to establish a connection
            return 0
        Whr.WaitForResponse()
        ;No Error Thrown
        if (Whr.ResponseText = "")
    ...
    GetAccountNames(){
        /*
        ;GETS
        ;Username,UserID,Alias,Description,Group,CSRFToken:=null,LastUsed,Cookie:=null,Fields
        ;Fields is a map 
        ;Fields are 
        ;SavePlaceId,SavedJobId
        ;This gets the Username for account 1 = Obj[1]["Username"]
        ;This gets SavedJobId for account 1 = Obj[1]["Fields"]["SavedJobId"]
        */
        url := 'http://localhost:' this.port '/GetAccountsJson?Password=' this.webserverpass ''
        Whr := ComObject("WinHTTP.WinHTTPRequest.5.1")
        Whr.Open("Get",url)
        ;establish a connection
        try Whr.Send()
        catch ;if failed to establish a connection return 0
            return 0
        Whr.WaitForResponse()   
        response := whr.ResponseText
        ;Response is a Array of Maps
        Obj := JSON.parse(response)
        ;Obj[1] = acc 1 
        ;Obj[2] = acc 2
        ;Obj[n]["UserId"] = User id for acc n
        ;Obj[n]["Username"] = User name for acc n
        AccountNames := []
        UserID := []    
        for account in Obj 
    ...
    GetJobid(Account) {
        ...
        ;establish a connection
        try Whr.Send()
        catch ;if failed to establish a connection return
            return 0
        Whr.WaitForResponse()   
        response := whr.ResponseText
        ;Response is a Array of Maps
